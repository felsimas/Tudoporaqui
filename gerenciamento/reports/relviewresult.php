...
<div id="page-wrapper">
	...
	<label><?=system_showText(LANG_SITEMGR_REPORT_LABEL_MONTH)?>: </label>
        ...
        <? if (($_GET["year"]) && ($_GET['month']) && is_numeric($_GET["year"]) && is_numeric($_GET['month'])) {
                $sql = "SELECT title, `listing_id`, SUM(`summary_view`) AS quantity FROM `Report_Listing_Monthly` inner join `Listing` on Listing.id = Report_Listing_Monthly.listing_id WHERE YEAR(`day`) = '".$_GET["year"]."' AND MONTH(`day`) = '".$_GET["month"]."' GROUP BY `listing_id` ORDER BY `quantity` DESC LIMIT 0, 30 ";
                $result = mysql_query($sql);
                if($result) {
                   ?>
